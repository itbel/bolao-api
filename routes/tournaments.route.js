...
router.patch("/join", verify, (req, res, next) => {
    ...
    tournamentModel.updateOne({ _id: req.body.tournamentid }, {
            $addToSet: {
                users: {
                    userid: req.user._id,
    ...
    tournamentModel.findOne({ _id: req.body.id }, (err1, doc1) => {
        if (err1) next(err1);
            ...
            if (req.user._id.toString() === doc1.owner.toString()) {
                tournamentModel.updateOne({ _id: req.body.id }, { status: "ENDED" },
                    (err2, doc2) => {
                        if (err2) next(err2);
                        else {
                            tournamentModel.findOne({ _id: req.body.id }, {
                                    users: [],
                                },
                                (err3, doc3) => {
                                    if (err3) next(err3);
                                        ...
                                        if (doc3 !== null) {
                                            ...
                                            userModel.findByIdAndUpdate({ _id: doc3.users[highestNum].userid }, {
                                                    $addToSet: {
...
router.get("/joined", verify, (req, res, next) => {
    ...
    tournamentModel.find({ "users.userid": req.user._id, status: "ACTIVE" },
        (err, doc) => {
            if (err) next(err);
            else {
            ...
            if (req.user._id.toString() === doc1.owner.toString()) {
                tournamentModel.deleteOne({ _id: req.params.id }, (err2, doc2) => {
                    if (err2) next(err2);
                        ...
                        matchModel.deleteMany({ tournamentid: req.params.id },
                            (err3, doc3) => {
                                if (err3) next(err3);
                                else {
                                    guessModel.deleteMany({ tournamentid: req.params.id },
                                        (err4, doc4) => {
                                            if (err4) next(err4);
                                            else {
                                                teamModel.deleteMany({ tournamentid: req.params.id },
                                                    (err5, doc5) => {
                                                        if (err5) next(err5);
                                                        else {
                                                            res.status(200).send();
